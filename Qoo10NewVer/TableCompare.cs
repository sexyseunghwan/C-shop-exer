...
namespace Qoo10NewVer
{
	class TableCompare
	{
		...
		public List<String[]> getErd()
		{
			...
			List<String[]> erdArr = new List<String[]>();//erd 테이블의 컬럼들이 들어갈 곳임.

			String[] inputErd = new String[3];//이름,형(크기),null 유무
			int indexErd = 0;

			String erdType = "";//형(크기)
			String nullAble = "";//null 유무

			while ((line = readerErd.ReadLine()) != null)
			{

				//다시 마음 가다듬고 코딩을 천천히 해봐
				line = line.ToLower().Trim();//소문자로 변환

				//테스트 용도
				//System.Console.WriteLine("=======");
				//System.Console.WriteLine(line);
				//System.Console.WriteLine(line.Equals(""));
				//System.Console.WriteLine("=======");

				//1. 이름을 지정
				if (indexErd % 3 == 0)
                {
					inputErd[0] = line;//이름 인서트
                }
				//2.형(크기) 
				else if (indexErd % 3 == 1)
                {
					if (!line.Equals("")) erdType = line;
		...
		public void tableCheck(List<String[]> inputArr)
		{
			...
			int innerLength = inputArr[0].Length;// 한 요소의 크기 --> 문제가 생김 여기서
